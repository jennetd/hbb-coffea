    ...
    for cat in cats:
        ...
        while fitfailed_qcd < 5:
            ...
            for ptbin in range(npt[cat]):
                ...
                qcdparams = np.array([rl.IndependentParameter('qcdparam_ptbin%d%s%s_%d' % (ptbin, cat, year, i), 0) for i in range(msd.nbins)])
                sigmascale = 10.
    ...
    for cat in cats:
        for ptbin in range(npt[cat]):
            ...
            for region in ['pass', 'fail']:
                ...
                data_obs = get_template('QCD', isPass, binindex+1, cat[:3]+'_', obs=msd, syst='nominal')
