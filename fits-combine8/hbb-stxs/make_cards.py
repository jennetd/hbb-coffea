...
def smass(sName):
    if 'ggF' in sName or 'VBF' in sName or sName in ['WH','ZH','ttH']:
        _mass = 125.
...
def ggfvbf_rhalphabet(tmpdir,
    ...
    with open('acc_ggF.json') as f:
        acc_ggF = json.load(f)
    with open('acc_VBF.json') as f:
        acc_VBF = json.load(f)
    ...
    pdf_acc_ggF = rl.NuisanceParameter('pdf_acceptance_ggF','lnN')
    pdf_acc_VBF = rl.NuisanceParameter('pdf_acceptance_VBF','lnN')
    ...
    sigs = ['ggF_s'+str(num) for num in range(1,4)]
    sigs += ['VBF_s'+str(num) for num in range(1,3)]
    samps = sigs + ['ggF_s0','VBF_s0','WH','ZH','ttH','Wjets','Zjets','Zjetsbb','EWKW','EWKZ','EWKZbb','ttbar','singlet','VV']
...
#                        print("Sample {} is too small, skipping".format(sName))
                    ...
                    if sName in sigs:
                        print("found " + sName, np.sum(nominal))
                    ...
                    if do_systematics:
                        ...
                        # Acceptance uncertainties on ggF and VBF
                        if sName in sigs:
                            ...
                            pname = str(int(sName[-1]) - 1)

                            if 'ggF' in sName:                       
                                for acc_unc in ['scalevar_7pt', 'PDFaS_weight', 'UEPS_FSR', 'UEPS_ISR']:
                                    sample.setParamEffect(acc_ggF_systs[acc_unc], 
                                                          acc_ggF[region][acc_unc][binname][pname][0], 
                                                          acc_ggF[region][acc_unc][binname][pname][1])
                                    
                            if 'VBF' in sName:
                                for acc_unc in ['scalevar_3pt', 'PDFaS_weight', 'UEPS_FSR', 'UEPS_ISR']:
                                    sample.setParamEffect(acc_VBF_systs[acc_unc],
                                                          acc_VBF[region][acc_unc][binname][pname][0],
                                                          acc_VBF[region][acc_unc][binname][pname][1])
                                    
                        # QCD scale and PDF uncertainties on Higgs signal    
                        elif sName in ['ggF_s0','VBF_s0','WH','ZH','ggZH','ttH']:
                    ...
                    # Add SFs last!
                    # DDB SF 
                    if sName in ['ggF_s2','ggF_s3','ggF_s4','VBF','WH','ZH','ggZH','ttH','Zjetsbb','EWKZbb']:
...
#                            sample.setParamEffect(sys_dict[sys], eff_up, eff_do)
                ...
                # DDB SF                                                                                  
                if sName in ['ggF_s2','ggF_s3','ggF_s4','VBF','WH','ZH','ggZH','ttH','Zjetsbb']:
