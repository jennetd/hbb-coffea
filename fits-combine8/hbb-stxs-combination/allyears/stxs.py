...
def get_ggF_table(logfile,normalized=False):
  ...
  w = np.array([0.5 for i in x])

  return rt.TGraphAsymmErrors(n,x,np.array(center),w,w,np.array(up_unc),np.array(do_unc))
...
if __name__ == "__main__":
    ...
    lumi = 138
    tag1 = rt.TLatex(0.53, 0.92, "%.0f fb^{-1} (13 TeV)" % lumi)
    tag1.SetNDC()
    tag1.SetTextFont(42)
    tag2 = rt.TLatex(0.19, 0.82, "CMS")
    tag2.SetNDC()
    tag2.SetTextFont(62)
    tag3 = rt.TLatex(0.19, 0.76, "H(bb) STXS stage 1.2")
    tag3.SetNDC()
    tag3.SetTextFont(42)
    ...
    h1.GetYaxis().SetRangeUser(0.7,8000)
    h1.GetXaxis().SetTitle("ggF p_{T}^{H} [GeV]")
    ...
    gggf = get_ggF_table(logfile = "logs-obs/fit_batch.out")
#    get_table(logfile = "logs-obs/fit_batch_stat.out")                                                                                                                        
    ...
    h2.SetLineColor(0)
    h2.SetLineWidth(3)
    h2.GetXaxis().SetTitle("VBF m_{jj}^{gen} [GeV]")
    h2.GetXaxis().CenterTitle(True)
    h2.GetXaxis().SetTitleOffset(2.5*pad3.GetAbsHNDC())
    h2.GetYaxis().SetRangeUser(0.7,8000)
    h2.Draw()
    ...
    gvbf = get_VBF_table(logfile = "logs-obs/fit_batch.out")
#    get_table(logfile = "logs-obs/fit_batch_stat.out")
    ...
    leg = rt.TLegend(0.2, 0.67, 0.9, 0.87)
    leg.SetBorderSize(0)
#    leg.SetTextFont(42)
    ...
    leg.AddEntry(gggf,"Observed (stat #oplus syst)","p")
    leg.AddEntry(g1,"ggF (HJ-MINLO)","f")
    ...
    grat = get_ggF_table(logfile = "logs-obs/fit_batch.out",normalized=True)
    ...
    grat2 = get_VBF_table(logfile = "logs-obs/fit_batch.out",normalized=True)
