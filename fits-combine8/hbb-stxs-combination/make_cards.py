...
def smass(sName):
    if sName in ['Wjets','EWKW','ttbar','singlet','VV']:
        _mass = 80.379
...
def ggfvbf_rhalphabet(tmpdir,
    ...
    with open('acceptance.json') as f:
        acceptance = json.load(f)
        
    with open('stxsnames.json') as f:
        stxsnames = json.load(f)
    stxsnames = dict(map(reversed, stxsnames.items()))
    with open('cmsnames.json') as f:
        cmsnames = json.load(f)
    ...
    pdf_Higgs_ggF = rl.NuisanceParameter('pdf_Higgs_accandndorm_ggF','lnN')
    pdf_Higgs_VBF = rl.NuisanceParameter('pdf_Higgs_accandnorm_VBF','lnN')
    pdf_Higgs_VH  = rl.NuisanceParameter('pdf_Higgs_accandnorm_VH','lnN')
    pdf_Higgs_ttH = rl.NuisanceParameter('pdf_Higgs_accandnorm_ttH','lnN')

    scale_ggF = rl.NuisanceParameter('QCDscale_accandnorm_ggF', 'lnN')
    scale_VBF = rl.NuisanceParameter('QCDscale_accandnorm_VBF', 'lnN')
    scale_VH = rl.NuisanceParameter('QCDscale_accandnorm_VH', 'lnN')
    scale_ttH = rl.NuisanceParameter('QCDscale_accandnorm_ttH', 'lnN')
    scale_EWKZ = rl.NuisanceParameter('QCDscale_EWKZ', 'lnN')

    isr_ggF = rl.NuisanceParameter('UEPS_ISR_accandnorm_ggF', 'lnN')
    isr_VBF = rl.NuisanceParameter('UEPS_ISR_acandnorm_VBF', 'lnN')
    isr_VH = rl.NuisanceParameter('UEPS_ISR_accandnorm_VH', 'lnN')
    isr_ttH = rl.NuisanceParameter('UEPS_ISR_accandnorm_ttH', 'lnN')

    fsr_ggF = rl.NuisanceParameter('UEPS_FSR_accandnorm_ggF', 'lnN')
    fsr_VBF = rl.NuisanceParameter('UEPS_FSR_accandnorm_VBF', 'lnN')
    fsr_VH = rl.NuisanceParameter('UEPS_FSR_accandnorm_VH', 'lnN')
    fsr_ttH = rl.NuisanceParameter('UEPS_FSR_accandnorm_ttH', 'lnN')

    pdf_acc_ggF = rl.NuisanceParameter('pdf_Higgs_acceptance_ggF','lnN')
    pdf_acc_VBF = rl.NuisanceParameter('pdf_Higgs_acceptance_VBF','lnN')
    pdf_acc_VH = rl.NuisanceParameter('pdf_Higgs_acceptance_VH','lnN')
    pdf_acc_ttH = rl.NuisanceParameter('pdf_Higgs_acceptance_ttH','lnN')

    scale_acc_ggF = rl.NuisanceParameter('QCDscale_acceptance_ggF', 'lnN')
    scale_acc_VBF = rl.NuisanceParameter('QCDscale_acceptance_VBF', 'lnN')
    scale_acc_VH = rl.NuisanceParameter('QCDscale_acceptance_VH', 'lnN')
    scale_acc_ttH = rl.NuisanceParameter('QCDscale_acceptance_ttH', 'lnN')

    isr_acc_ggF = rl.NuisanceParameter('UEPS_ISR_acceptance_ggF', 'lnN')
    isr_acc_VBF = rl.NuisanceParameter('UEPS_ISR_acceptance_VBF', 'lnN')
    isr_acc_VH = rl.NuisanceParameter('UEPS_ISR_acceptance_VH', 'lnN')
    isr_acc_ttH = rl.NuisanceParameter('UEPS_ISR_acceptance_ttH', 'lnN')

    fsr_acc_ggF = rl.NuisanceParameter('UEPS_FSR_acceptance_ggF', 'lnN')
    fsr_acc_VBF = rl.NuisanceParameter('UEPS_FSR_acceptance_VBF', 'lnN')
    fsr_acc_VH = rl.NuisanceParameter('UEPS_FSR_acceptance_VH', 'lnN')
    fsr_acc_ttH = rl.NuisanceParameter('UEPS_FSR_acceptance_ttH', 'lnN')

    acc_ggF_systs = {'scalevar_7pt':scale_acc_ggF,'PDFaS_weight':pdf_acc_ggF,'UEPS_FSR':fsr_acc_ggF,'UEPS_ISR':isr_acc_ggF}
    acc_VBF_systs = {'scalevar_3pt':scale_acc_VBF,'PDFaS_weight':pdf_acc_VBF,'UEPS_FSR':fsr_acc_VBF,'UEPS_ISR':isr_acc_VBF}
    acc_VH_systs = {'scalevar_3pt':scale_acc_VH,'PDFaS_weight':pdf_acc_VH,'UEPS_FSR':fsr_acc_VH,'UEPS_ISR':isr_acc_VH}
    acc_ttH_systs = {'scalevar_7pt':scale_acc_ttH,'PDFaS_weight':pdf_acc_ttH,'UEPS_FSR':fsr_acc_ttH,'UEPS_ISR':isr_acc_ttH}
    ...
    sigs = ['s'+str(m)+'0'+str(s) for m in range(1,9) for s in range(0,10)]
    sigs += ['s'+str(m)+str(s) for m in range(1,9) for s in range(10,28)]
    samps = sigs + ['Wjets','Zjets','Zjetsbb','EWKW','EWKZ','EWKZbb','ttbar','singlet','VV']    
...
#                        print("Sample {} is too small, skipping".format(sName))
                    ...
                    samplename = sName
                    if sName in sigs:
                        samplename = cmsnames[stxsnames[int(sName[1:])]]
                        print(samplename)

                    sample = rl.TemplateSample(ch.name + '_' + samplename, stype, templ)
                    ...
                    if do_systematics:
                        ...
                        for sys in exp_systs:
                            ...
                            eff_up = max(shape_to_num(syst_up,nominal),0.0001)
                            eff_do = max(shape_to_num(syst_do,nominal),0.0001)
                        ...
                        if sName in ['Wjets']:
                            ...
                            sample.setParamEffect(scale_EWKZ,eff_scale_up,eff_scale_do)

                        # Acceptance uncertainties
                        if 0: #sName in sigs:

                            binname = cat+'-pt'+str(binindex+1)
                            if 'vbflo' in cat:
                                binname = 'vbf-mjj1'
                            if 'vbfhi' in cat:
                                binname = 'vbf-mjj2'
                            ...
                            eff_fsr_up = max(np.sum(fsr_up)/np.sum(nominal),0.0001)
                            eff_fsr_do = max(np.sum(fsr_do)/np.sum(nominal),0.0001)
                            ...
                            eff_isr_up = max(np.sum(isr_up)/np.sum(nominal),0.0001)
                            eff_isr_do = max(np.sum(isr_do)/np.sum(nominal),0.0001)
                            ...
                            eff_pdf_up = max(np.sum(pdf_up)/np.sum(nominal),0.0001)
                            eff_pdf_do = max(np.sum(pdf_do)/np.sum(nominal),0.0001)

                            if int(sName[1]) == 1:
                                for k, a in acc_ggF_systs.items():
                                    syst_up = max(acceptance[region][k][binname][str(int(pname)%100)][0],0.0001)
                                    syst_do = max(acceptance[region][k][binname][str(int(pname)%100)][1],0.0001)
                                    sample.setParamEffect(a, syst_up, syst_do)

                                scale_up = get_template(sName, isPass, binindex+1, cat[:3]+'_', obs=msd, syst='scalevar_7ptUp')[0]
                                scale_do = get_template(sName, isPass, binindex+1, cat[:3]+'_', obs=msd, syst='scalevar_7ptDown')[0]

                                eff_scale_up = max(np.sum(scale_up)/np.sum(nominal),0.0001)
                                eff_scale_do = max(np.sum(scale_do)/np.sum(nominal),0.0001)
                                ...
                                for k, a in acc_VBF_systs.items():
                                    syst_up = max(acceptance[region][k][binname][str(int(pname)%100)][0],0.0001)
                                    syst_do = max(acceptance[region][k][binname][str(int(pname)%100)][1],0.0001)
                                    sample.setParamEffect(a, syst_up, syst_do)
                            ...
                            elif int(sName[1]) == 3 or int(sName[1]) == 4 or int(sName[1]) == 5:
                                for k, a in acc_VH_systs.items():
                                    syst_up = max(acceptance[region][k][binname][str(int(pname)%100)][0],0.0001)
                                    syst_do = max(acceptance[region][k][binname][str(int(pname)%100)][1],0.0001)
                                    sample.setParamEffect(a, syst_up, syst_do)
                                ...
                                for k, a in acc_ttH_systs.items():
                                    syst_up = max(acceptance[region][k][binname][str(int(pname)%100)][0],0.0001)
                                    syst_do = max(acceptance[region][k][binname][str(int(pname)%100)][1],0.0001)
                                    sample.setParamEffect(a, syst_up, syst_do)
