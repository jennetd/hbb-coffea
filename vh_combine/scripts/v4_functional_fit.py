...
# Read the histogram from root file
...
# Return the histogram from the root file instead of the template
def get_template_hist(sName, passed, ptbin, cat, obs, syst, muon=False):
...
def create_pass_QCD(fit_result, fit_error, qcdeff, sName, passed, ptbin, cat, obs, syst, muon=False):

    """Create the passing QCD histogram from the fitted parameter
    Assuming 3rd degree poly
    """
    ...
    print("Creating pass template from fitted parameters for: ", name)
    ...
    for i in range(1,h.GetNbinsX()+1):
        ...
        fitted_qcd = qcdeff*fit_result(msd_bin_center)
        y_value.append(fitted_qcd)
        y_err.append(qcdeff*fit_error.GetBinError(i))
    ...
    plt.ylabel('Events')
    plt.legend()
    plt.savefig("plttest_{}.pdf".format(name))
    plt.close()


    #Do the usual stuff in get_template to return the template
    sumw = []
    sumw2 = []
...
def fit_fail_templ_QCD(sName, passed, ptbin, cat, obs, syst, muon=False):
    ...
    fit_function = ROOT.TF1("fa1","[0] + [1]*x + [2]*x*x + [3]*x*x*x",40,200)
    ...
    for cat in cats:
        ...
        while fitfailed_qcd < 5: #Fail if choose bad initial values, start from where the fits fail. 
            ...
            for ptbin in range(npt[cat]):
                ...
                failTempl = fit_fail_templ_QCD('QCD', 0, binindex+1, cat, obs=msd, syst='nominal')
