...
def fit_fail_templ_QCD(sName, passed, ptbin, cat, obs, syst, muon=False):
    ...
    for i in range(1,h.GetNbinsX()+1):
        ...
        sum_og += h.GetBinContent(i)
        sum_fit += fit_result(msd_bin_center)

        fitted_qcd = fit_result(msd_bin_center)
        y_value.append(fitted_qcd)
        y_err.append(fit_error.GetBinError(i))

        print("msd bin center", msd_bin_center)
        print("OG bin value:", h.GetBinContent(i))
        print("Fitted bin value:",fit_result(msd_bin_center))

        sumw += [fitted_qcd]
        sumw2 += [fit_error.GetBinError(i)*fit_error.GetBinError(i)]

    print("OG integral: ", sum_og)
    print("Fitted integral: ", sum_fit)
