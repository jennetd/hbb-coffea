...
def main():
    ...
    sig = pickle.load(open(picklename,'rb')).integrate('region','signal').integrate('ddc2',int_range=slice(0,ddcthr)).sum(*['genflavor1', 'genflavor2', 'msd2'], overflow='all')
    ...
    for p in samples:
        ...
        for s in hfail.identifiers('systematic'):
            ...
            fout["l_pass_"+p+"_"+str(s)] = hist.export1d(hpass.integrate('systematic',s))
            fout["l_fail_"+p+"_"+str(s)] = hist.export1d(hfail.integrate('systematic',s))
