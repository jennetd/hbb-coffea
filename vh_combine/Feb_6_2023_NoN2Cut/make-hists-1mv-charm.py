...
def main():
    ...
    if os.path.isfile(year+'/1mvc-signalregion.root'):
        os.remove(year+'/1mvc-signalregion.root')
    fout = uproot3.create(year+'/1mvc-signalregion.root')

    samples = ['data','muondata','EWKW', 'EWKZ', 'Wjets', 'Zjets', 'QCD','ttbarBoosted','singlet','VV','ggF','VBF','WH','ZH','ttH']

    #print("0 V MASS BINS, CHARM CAT")
    #bins = [40,201]
    ...
    sig = pickle.load(open(picklename,'rb')).integrate('region','signal').integrate('ddc2',int_range=slice(ddcthr,1)).sum(*['genflavor1', 'genflavor2', 'msd2'], overflow='all')
    ...
    for p in samples:
        ...
        for s in hfail.identifiers('systematic'):
            ...
            fout["c_pass_"+p+"_"+str(s)] = hist.export1d(hpass.integrate('systematic',s))
            fout["c_fail_"+p+"_"+str(s)] = hist.export1d(hfail.integrate('systematic',s))
