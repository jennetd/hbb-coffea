...
def main():
    ...
    if os.path.isfile(year+'/3mvc-signalregion.root'):
        os.remove(year+'/3mvc-signalregion.root')
    fout = uproot3.create(year+'/3mvc-signalregion.root')

    samples = ['data','muondata','QCD','ttbar','singlet','VV','ggF','VBF','WH','ZH','ttH']

    print("3 V MASS BINS, CHARM CAT")
    bins = [40,68,103,201]
    ...
    sig = pickle.load(open(picklename,'rb')).integrate('region','signal').sum('genflavor2').integrate('ddc2',int_range=slice(ddcthr,1))
