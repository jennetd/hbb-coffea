...
def main():
    ...
    if os.path.isfile(year+'/3mvl-signalregion.root'):
        os.remove(year+'/3mvl-signalregion.root')
    fout = uproot3.create(year+'/3mvl-signalregion.root')

    samples = ['data','muondata','QCD','ttbar','singlet','VV','ggF','VBF','WH','ZH','ttH']

    print("3 V MASS BINS, LIGHT CAT")
    bins = [40,68,103,201]
